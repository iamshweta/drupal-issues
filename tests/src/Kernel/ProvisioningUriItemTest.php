...
class ProvisioningUriItemTest extends KernelTestBase {
  ...
  public function testEmptyFieldItemListPasswordException() {
    $this->expectException(MissingProvisioningUriException::class);
    $this->expectExceptionMessage('Cannot get password, provisioning field is empty.');
  ...
  public function testEmptyFieldItemUriPasswordException() {
    $this->expectException(MissingProvisioningUriException::class);
    $this->expectExceptionMessage('Cannot get password, uri property on provisioning field is empty.');
    $this->entity->test_field = [
      'uri' => '',
    ];
    $this->entity->test_field->getOneTimePassword();
