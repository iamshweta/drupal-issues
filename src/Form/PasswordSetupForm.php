...
class PasswordSetupForm extends FormBase {
  ...
  public function enableTwoFactorAuth(array &$form, FormStateInterface $form_state) {
    ...
    $user->save();
    $this->messenger()->addMessage($this->t('Two-factor authentication has been enabled. See the instructions below for setting up your one time password.'));
  }
  ...
  public function disableTwoFactorAuth(array &$form, FormStateInterface $form_state) {
    ...
    $user->save();
    $this->messenger()->addMessage($this->t('Two-factor authentication has been disabled for this account.'));
  }
